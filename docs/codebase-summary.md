...
- **Legacy global mode (`csp use [name]`)**: mutates `~/.claude` and `.active` using physical profile snapshots, including `default`, with `default` used when no name is provided.
...
- **CLI lifecycle utilities**: `csp exec` runs arbitrary commands inside an isolated runtime and `csp update` updates the installed CLI with auto-detected or explicit install method.

## Project Structure

```
bin/csp.js                          → CLI entry point (commander.js, 18 commands)
src/commands/                       → select, init, current, list, status, create, save, use,
                                      toggle, delete, export, import, diff, deactivate,
                                      launch, exec, uninstall, update
...
- `csp init` creates a physical `default` snapshot in `~/.claude-profiles/default`; `save`, `use`, `export`, and `launch` operate on that snapshot.
...
- `csp use [name]` defaults to `default` and fails fast while Claude Code is running.
- `csp select` is the default command — interactive arrow-key profile picker that delegates to `launch`.
- `csp exec <name> -- <command> [args...]` runs arbitrary commands inside an isolated profile runtime without mutating global `~/.claude`.
- `csp update` updates the installed CLI, auto-detecting `standalone`, `brew`, or `npm` unless `--method` is provided; `--force` skips confirmation.
